...
function MiniCandle({
  ...
  if (wickMode === "top" && typeof wickTopLenPx === "number") {
    ...
    wickY1 = yTopFromCore; wickY2 = bodyTopY + (wickJoinPx || 0);
  } else if (wickMode === "bottom") {
    wickY1 = bodyBottomY - (wickJoinPx || 0); wickY2 = yBottomFromCore;
  }
  ...
  return (
    <svg width={VIEW_WIDTH * SVG_SCALE} height={VIEW_HEIGHT * SVG_SCALE} viewBox={`0 0 ${VIEW_WIDTH} ${VIEW_HEIGHT}`}>
      ...
      <rect x="8" y={bodyY} width="12" height={bodyH} fill={bodyColor} stroke={bodyColor === "#3fb950" ? "#238636" : bodyColor === "#f85149" ? "#da3633" : "#484f58"} strokeWidth="0.5" rx="1.5" />
    </svg>
  );
}
...
const candles = [
  {
    name: "Hammer", deName: "Hammer",
    merk: "Abverkauf wird gekauft – Käufer übernehmen.",
    color: "green", typ: "Bullish Reversal", markt: "Abwärtstrend",
  ...
  {
    name: "Dragonfly Doji", deName: "Libellen-Doji",
    ...
    wirkung: "2–3", staerke: 3,
    svg: () => <DragonflyDojiSVG />,
  },
  {
    name: "Gravestone Doji", deName: "Grabstein-Doji",
  ...
  {
    name: "Inverted Hammer", deName: "Invertierter Hammer",
    merk: "Hoch wird getestet – Käufer zeigen erste Stärke.",
    color: "green", typ: "Bullish Reversal (schwach)", markt: "Abwärtstrend",
    ...
    wirkung: "2–3", staerke: 4,
    svg: () => <MiniCandle bodyColor="#3fb950" padding={4} wickMode="top" wickTopLenPx={25} wickJoinPx={1} core={{ top: 3.3, bottom: 1.0, bodyY: 1.15, bodyH: 0.55 }} />,
  },
  {
  ...
  {
    name: "Marubozu (rot)", deName: "Marubozu (rot)",
    merk: "Nur Verkaufsdruck – Abwärtsdruck ohne Pause.",
    color: "red", typ: "Bearish Continuation", markt: "Abwärtstrend",
  ...
  {
    name: "Doji", deName: "Doji",
    merk: "Unentschlossenheit – Folgekerze entscheidet.",
    color: "gray", typ: "Neutral", markt: "Trendphase beliebig",
  ...
  {
    name: "Spinning Top", deName: "Kreisel",
    merk: "Pause im Trend – Kraft sammelt sich.",
    color: "gray", typ: "Neutral / Trendpause", markt: "Seitwärtsphase oder Trendpause",
...
// ─── Context Chart (neu: ein gemeinsames SVG, echte Preisskala) ───────────────
//
// Jede Kerze wird als { high, low, open, close, color } definiert – reale Preiswerte.
// Alle Kerzen teilen denselben SVG-Canvas mit einer gemeinsamen Y-Achse.
// Kein translateY-Hack mehr.

const GREEN = "#3fb950";
const RED   = "#f85149";
const GRAY  = "#7d8590";

// Definiert Kontext-Sequenzen per Muster.
// Preiseinheiten sind frei wählbar – wichtig ist nur die relative Relation.
// Format: { high, low, open, close, color }
// "signal" markiert die Hauptkerze (wird gelb umrandet)
function getChartSequence(name) {
  switch (name) {

    case "Hammer":
      // Abwärtstrend + Hammer unten + grüne Bestätigung
      return [
        { high: 110, low: 96,  open: 109, close: 98,  color: RED   },
        { high: 100, low: 88,  open: 99,  close: 90,  color: RED   },
        { high: 92,  low: 76,  open: 91,  close: 88,  color: GREEN, signal: true }, // langer unterer Schatten, kleiner grüner Körper
        { high: 94,  low: 86,  open: 88,  close: 93,  color: GREEN },
      ];

    case "Hanging Man":
      // Aufwärtstrend + Hanging Man oben + rote Bestätigung
      return [
        { high: 92,  low: 82,  open: 83,  close: 91,  color: GREEN },
        { high: 100, low: 90,  open: 91,  close: 99,  color: GREEN },
        { high: 101, low: 85,  open: 100, close: 97,  color: RED,   signal: true }, // langer unterer Schatten, kleiner Körper oben
        { high: 98,  low: 88,  open: 97,  close: 89,  color: RED   },
      ];

    case "Inverted Hammer":
      // Abwärtstrend + Invertierter Hammer + grüne Bestätigung
      return [
        { high: 112, low: 98,  open: 111, close: 100, color: RED   },
        { high: 102, low: 88,  open: 101, close: 90,  color: RED   },
        { high: 100, low: 84,  open: 86,  close: 88,  color: GREEN, signal: true }, // langer oberer Schatten, kleiner Körper unten
        { high: 96,  low: 86,  open: 88,  close: 95,  color: GREEN },
      ];

    case "Shooting Star":
      // Aufwärtstrend + Shooting Star + rote Bestätigung
      return [
        { high: 94,  low: 84,  open: 85,  close: 93,  color: GREEN },
        { high: 102, low: 91,  open: 93,  close: 101, color: GREEN },
        { high: 116, low: 99,  open: 101, close: 103, color: RED,   signal: true }, // langer oberer Schatten, kleiner Körper unten
        { high: 104, low: 94,  open: 103, close: 95,  color: RED   },
      ];

    case "Dragonfly Doji":
      // Abwärtstrend + Dragonfly (T-Form) + grüne Bestätigung
      return [
    ...
    case "Gravestone Doji":
      // Aufwärtstrend + Gravestone (inverses T) + rote Bestätigung
      return [
    ...
    case "Doji":
      // Neutral: gemischter Trend + Doji (kein Confirm)
      return [
    ...
    case "Spinning Top":
      ...
      return [
        ...
        { high: 106, low: 90,  open: 98,  close: 100, color: GREEN, signal: true }, // kleiner Körper, langer ober+unter Schatten
      ];

    case "Marubozu (grün)":
      // Starker Aufwärtstrend + Marubozu ohne Schatten
      return [
    ...
    case "Marubozu (rot)":
      // Starker Abwärtstrend + roter Marubozu ohne Schatten
      return [
...
function ContextChart({ candle }) {
  ...
  // Preisskala: globales min/max über alle Kerzen
  ...
  const trendLabel = markt.includes("aufwärt") ? "↑ Aufwärtstrend"
    : markt.includes("abwärt") ? "↓ Abwärtstrend"
  ...
  return (
    <div style={{ marginTop: 12 }}>
      ...
      <div style={{
        ...
        padding: "8px 4px 4px", display: "inline-block",
      }}>
            ...
            return (
              <g key={i}>
                {isSignal && (
                  <rect
                    x={x - 3} y={yHigh - 4}
                    width={candleW + 6} height={yLow - yHigh + 8}
                    rx={3} ry={3}
                    fill={`${C.yellow}12`}
                    stroke={C.yellow}
                    strokeWidth="1"
                ...
                {/* Doji: horizontaler Strich statt Körper */}
                {c.doji ? (
                  <line
                    x1={x + 2} x2={x + candleW - 2}
                    y1={yTop} y2={yTop}
                    stroke={GRAY}
                    strokeWidth="2"
                    strokeLinecap="round"
  ...
  for (let i = 0; i < 50; i++) {
    ...
    switch (qtype) {
      ...
      case "strategy_from_name":
        ...
        correct = item.strategie; choices = [correct, ...pick(candles.filter(c => c.name !== item.name).map(c => c.strategie), 3)];
        renderGraphic = item.svg; break;
      default: break;
    }
...
export default function App() {
  ...
  function stopReview() {
    ...
    setRound({ queue: buildRoundQuestions(ROUND_SIZE), index: 0 }); setAnswer(null); setFlash(null);
  }
  function resetAll() {
    setScore(0); setHistory([]); setLastRound(null); setReviewMode(false);
    setReviewQueue([]); setReviewIndex(0);
    setRound({ queue: buildRoundQuestions(ROUND_SIZE), index: 0 }); setAnswer(null); setFlash(null);
  }
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", display: "grid", gap: 16 }}>
          ...
          <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap", paddingBottom: 16, borderBottom: `1px solid ${C.border}` }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              ...
              <button className="nav-btn" onClick={resetAll}>Reset</button>
              ...
              <div style={{ display: "grid", gap: 20 }}>
                ...
                {answer && (
                  <div className="reveal-box">
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 20px" }}>
                        {[
                          ...
                          <div key={label} className="info-row">
                            ...
                            <span className="info-value" style={{ fontSize: 13 }}>{val}</span>
                          </div>
                        ))}
                      </div>
                      <div className="info-row" style={{ flexDirection: "column", gap: 2 }}>
                        <span className="info-label">Strategie</span>
                        <span className="info-value" style={{ fontSize: 13, color: C.textMuted }}>{q.item.strategie}</span>
                      </div>
                    </div>
                    ...
                    {q.item.erklaerung && (
                      <div style={{ marginTop: 12, padding: "10px 14px", borderRadius: 6, background: C.blueBg, border: `1px solid ${C.blueBdr}`, fontSize: 13, lineHeight: 1.6, color: C.textMuted }}>
                        <span style={{ color: C.blue, fontWeight: 600 }}>Erklärung: </span>{q.item.erklaerung}
                      </div>
                    )}
                  </div>
                )}
                ...
                return (
                  <div key={i} className="flash-card">
                    <div style={{ display: "flex", gap: 14, alignItems: "flex-start", marginBottom: 12 }}>
                      <div style={{ flexShrink: 0, background: C.surfaceHi, borderRadius: 8, padding: "8px 6px", border: `1px solid ${C.border}` }}>
                        {c.svg && <c.svg />}
                      </div>
                    ...
                    <div style={{ display: "grid", gap: 6 }}>
                      {[
                        ...
                        <div key={label} className="info-row">
                          <span className="info-label">{label}</span>
                          <span className="info-value" style={{ fontSize: 12, color: C.textMuted }}>{val}</span>
                        </div>
                      ))}
                    </div>
                    <ContextChart candle={c} />
                    {c.erklaerung && (
                      <div style={{ marginTop: 10, padding: "8px 12px", borderRadius: 6, background: C.blueBg, border: `1px solid ${C.blueBdr}`, fontSize: 12, color: C.textMuted, lineHeight: 1.5 }}>
                        <span style={{ color: C.blue, fontWeight: 600 }}>Erklärung: </span>{c.erklaerung}
                      </div>
                    )}
                  </div>
                );
            ...
            <div className="card">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                  <div style={{ fontFamily: font.mono, fontSize: 22, fontWeight: 700, color: lastRound.percent >= 70 ? C.green : lastRound.percent >= 40 ? C.yellow : C.red }}>
                    {lastRound.percent}%
                  </div>
                  <button
                    className="nav-btn"
              ...
              <div style={{ overflowX: "auto" }}>
                <table>
                  <thead>
                    <tr>
                      <th>#</th><th>Kerze</th><th>Frage (kurz)</th><th>Ergebnis</th><th>Punkte</th>
                    </tr>
                  </thead>
                  <tbody>
                    {lastRound.entries.map((e, idx) => {
                      ...
                      return (
                        <tr key={e.nr}>
                          ...
                          <td style={{ color: C.textMuted, fontSize: 12 }}>{prompt.length > 55 ? prompt.slice(0, 52) + "…" : prompt}</td>
                          <td style={{ color: e.isCorrect ? C.green : C.red, fontFamily: font.mono, fontSize: 13 }}>{e.isCorrect ? "✓" : "✗"}</td>
                          <td style={{ fontFamily: font.mono, fontSize: 12, color: e.isCorrect ? C.green : C.textMuted }}>{e.points}</td>
                        </tr>
                      );
                    })}
                    <tr style={{ fontWeight: 700 }}>
                      <td colSpan={3} style={{ color: C.textMuted, paddingTop: 12 }}>Gesamt</td>
                      <td style={{ paddingTop: 12, color: lastRound.percent >= 70 ? C.green : lastRound.percent >= 40 ? C.yellow : C.red }}>
