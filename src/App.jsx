...
const candles = [
  ...
  },
  {
    ...
    strategie: "Nach einem starken Abwärtsschub kommt oft ein kurzer Pullback – Käufer versuchen sich zu erholen, scheitern aber. Genau dort steigen Swing-Trader short ein: besserer Preis, kleineres Risiko. Stop über dem Pullback-Hoch, Ziel: nächste Unterstützungszone.",
    wirkung: "3–5",
    staerke: 8,
...
export default function App() {
  ...
  useEffect(() => {
    const f=[{q:"Was ist ein Candlestick Muster?",a:"Kerzenmuster zeigen das Kursverhalten. Muster wie Hammer oder Doji signalisieren Trendwechsel."},{q:"Für wen ist das Quiz?",a:"Für Einsteiger und Fortgeschrittene."},{q:"Wie viele Muster?",a:"10: Hammer, Inverted Hammer, Hanging Man, Shooting Star, Dragonfly Doji, Gravestone Doji, Marubozu grün/rot, Doji, Spinning Top."},{q:"Kostenlos?",a:"Ja, vollständig kostenlos."},{q:"Hammer vs Hanging Man?",a:"Identisch, aber: Hammer bullisch nach Abwärtstrend, Hanging Man bärisch nach Aufwärtstrend."},{q:"Bestätigung?",a:"Durch die Folgekerze. RSI und Volumen helfen."}];
      ...
      <div className="app-padding" style={{ minHeight: "100vh", background: C.bg, color: C.text, padding: "20px 20px 48px", fontFamily: font.sans }}>
        <div style={{ maxWidth: 900, margin: "0 auto", display: "grid", gap: 16 }}>
          ...
          <section style={{ display:"grid", gap:0 }}>
            ...
            {[{q:"Was ist ein Candlestick Muster?",a:"Ein Candlestick Muster zeigt das Kursverhalten in einem bestimmten Zeitraum. Muster wie Hammer, Doji oder Shooting Star signalisieren mögliche Trendwechsel."},{q:"Für wen ist das Quiz?",a:"Für Einsteiger die Trading lernen möchten, sowie Trader die ihr Wissen auffrischen wollen."},{q:"Wie viele Muster?",a:"10 Muster: Hammer, Inverted Hammer, Hanging Man, Shooting Star, Dragonfly Doji, Gravestone Doji, Marubozu grün/rot, Doji und Spinning Top."},{q:"Ist das Quiz kostenlos?",a:"Ja, vollständig kostenlos und ohne Anmeldung nutzbar."},{q:"Unterschied Hammer und Hanging Man?",a:"Beide sehen identisch aus. Hammer erscheint nach Abwärtstrend (bullisch), Hanging Man nach Aufwärtstrend (bärisch)."},{q:"Wie wird ein Muster bestätigt?",a:"Durch die Folgekerze die die Richtung bestätigt. RSI und Volumen helfen zusätzlich."}].map(({q,a},i)=><FaqItem key={i} question={q} answer={a}/>)}
          </section>
        ...
        {showCertificate && certLevel && (() => {
          const lv={bronze:{icon:"🥉",titel:"Trading Einsteiger",farbe:"#cd7f32",msg:"Du hast dein erstes Ziel erreicht! Deine Candlestick-Grundlagen sitzen."},silber:{icon:"🥈",titel:"Trading Fortgeschrittener",farbe:"#a8a8a8",msg:"Beeindruckend! Du beherrschst Candlestick Muster auf fortgeschrittenem Niveau."},gold:{icon:"🥇",titel:"Trading Profi",farbe:"#e3b341",msg:"Außergewöhnlich! Du hast das höchste Level erreicht – wie ein echter Trading-Profi."}}[certLevel];
          return (
          <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000, padding:20 }}>
            <div className="cert-popup" style={{ background:C.surface, border:`2px solid ${lv.farbe}`, borderRadius:14, padding:28, maxWidth:440, width:"100%", display:"grid", gap:16, maxHeight:"90vh", overflowY:"auto" }}>
              {!certSent ? (
                <>
                  <div style={{ fontSize:48, textAlign:"center" }}>{lv.icon}</div>
                  <div style={{ fontSize:20, fontWeight:600, textAlign:"center", color:lv.farbe }}>Glückwunsch – {lv.titel}!</div>
                  <p style={{ fontSize:15, color:C.textMuted, lineHeight:1.6, textAlign:"center" }}>{lv.msg}</p>
                  <div style={{ padding:"10px 14px", borderRadius:8, background:C.surfaceHi, border:`1px solid ${C.border}`, fontSize:13, color:C.textMuted, textAlign:"center" }}>Füll das Formular aus und erhalte dein <strong style={{ color:C.text }}>Zertifikat</strong> kostenlos per E-Mail.</div>
                  {[["Vorname",certFirstName,setCertFirstName,"z. B. Max"],["Nachname",certLastName,setCertLastName,"z. B. Mustermann"],["E-Mail",certEmail,setCertEmail,"deine@email.de"]].map(([label,val,set,ph])=>(
                    <div key={label} style={{ display:"grid", gap:5 }}>
                      ...
                      <input type={label==="E-Mail"?"email":"text"} placeholder={ph} value={val} onChange={e=>set(e.target.value)} style={inputStyle}/>
                    </div>
                  ))}
                  <div style={{ display:"grid", gap:5 }}>
                    <label style={{ fontSize:11, color:C.textMuted, fontWeight:600 }}>Geburtsdatum <span style={{ fontSize:11, fontWeight:400 }}>(optional)</span></label>
                    <input type="date" value={certBirthdate} onChange={e=>setCertBirthdate(e.target.value)} style={inputStyle}/>
                  </div>
                  <div style={{ display:"grid", gap:5 }}>
                    <label style={{ fontSize:11, color:C.textMuted, fontWeight:600 }}>Postleitzahl <span style={{ fontSize:11, fontWeight:400 }}>(optional)</span></label>
                    <input type="text" placeholder="z. B. 22297" value={certPlz} onChange={e=>setCertPlz(e.target.value.replace(/[^0-9]/g,"").slice(0,5))} maxLength={5} style={inputStyle}/>
                  </div>
                  ...
                  <div style={{ display:"flex", gap:10 }}>
                    <button className="nav-btn primary" style={{ flex:1, opacity:(!certFirstName||!certLastName||!certEmail||!certPrivacy)?0.4:1 }}
                      ...
                      onClick={()=>{
                        ...
                        const body=`Hallo,\n\nich habe das ${lv.titel}-Zertifikat im Candlestick Quiz erreicht.\n\nName: ${certFirstName} ${certLastName}\nE-Mail: ${certEmail}\nStufe: ${lv.icon} ${lv.titel}${geburt}${plz}\nDatum: ${heute}\n\nViele Grüße\n${certFirstName} ${certLastName}`;
                ...
                <>
                  ...
                  <p style={{ fontSize:15, color:C.textMuted, lineHeight:1.6, textAlign:"center" }}>Bitte auf <strong style={{ color:C.text }}>Senden</strong> klicken.</p>
